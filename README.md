...
- 进入下一稀疏段前仍要求六轴真实反馈稳定到位；仅收到新 `/joint_states` 消息时计数。
- 筛选同时保留六轴的转向和姿态变化，避免丢失纯腕部往返动作。
- 小于平移到位容差的轴不参与腕部进度计算；段内进度不回退，平移到位后补发完整腕部终点。
...
默认控制周期由 5 Hz 调整为 10 Hz；可用 `controller_update_rate:=5/10/20`
做对照。`stm32_state_poll_hz` 是独立串口轮询频率，不等于控制器更新频率。
例如在原启动命令末尾增加 `controller_update_rate:=10`；串口、方向、零偏
继续沿用实机已经确认的参数。20 Hz 尚未经过实机验证。

本轮保留段间到位等待，没有实现连续通过中间点。腕部关键点可能增加
保留段数，这是为了保留原始姿态路径。稳定计数表示不同 ROS 状态消息，
不表示每个电机获得了新的硬件采样；现有协议未提供逐轴采样序号。

验证：纯 Python 回归覆盖微小平移量、正反向容差收尾、进度抖动、腕部往返
路径和重复反馈计数；实际平滑度与驱动器吞吐需实机确认。
